...
本書では、「緑の評価指標算出機能」の利用環境構築手順について記載しています。優良緑地確保計画認定制度(TSUNAG)との連携を想定し、緑化の評価指標を算出し、指定したエリア内での緑地面積の自動算出機能や日陰シミュレーションの可視化、登録された緑地データに基づくCO₂吸収量の算出機能などを提供します。
...
3D都市モデル（3DTiles、建築物モデル・植生モデル等）を Amazon S3 に格納します。対象は、技術検証レポートにてご確認ください。
3D都市モデル（3DTiles） は、[G空間情報センター](https://front.geospatial.jp/)からダウンロードしてください。植生モデル（LOD1,2）のCityGML、3DTilesは、本リポジトリの[FMEワークスペース](https://github.com/Project-PLATEAU/green-indicator/tree/main/workspaces)で作成することが可能です。
