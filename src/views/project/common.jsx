...
import { Box, styled, Typography, OutlinedInput, InputAdornment, IconButton } from "@mui/material"
...
export const NumberTextField = (props) => {
    ...
    const onCopyCalced = useCallback(() => {
        if (!props.calced) { return }
        onChange({ target: { value: props.calced } })
        if (props.onCopyCalced) { return props.onCopyCalced() }
    }, [props.calced, props.onChange])
    ...
    return (
        <BaseTextField
            ...
            startAdornment={props.calced ? (
                <InputAdornment position="start">
                    <IconButton size="small" sx={nStyles.copy.button} onClick={onCopyCalced}><StartIcon sx={nStyles.copy.icon} /></IconButton>
                </InputAdornment>
            ) : null}
            endAdornment={props.unit ? (<InputAdornment position="end">{props.unit}</InputAdornment>) : null}
            placeholder={props.calced}
            {...props}
            value={value}
            onChange={onChange}
            onClick={onClick}
